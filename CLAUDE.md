...
- **⚠️ 가짜 데이터 DB 삽입 전면 금지 (확장 규칙):**
  - `hashlib`, `random`, 임의 수식 등으로 생성한 값을 DB에 삽입하는 패턴을 **API 성공/실패 여부와 무관하게 전면 금지**합니다.
  - "Deterministic mock", "simulation", "fallback" 등 어떤 이름으로 포장되더라도 동일하게 금지입니다.
  - 실패 시에는 `logger.warning`으로 기록 후 `return`으로 스킵합니다. 가짜 데이터가 실데이터처럼 취급되는 오염을 방지합니다.
  - **유일한 예외:** 실 API 자체가 존재하지 않는 필드(예: `kosis_vacancy_rate`, `disaster_risk_ratio`)만 대체 계산값 사용이 허용되며, 반드시 해당 필드 옆에 `# 실API 없음` 주석을 명시합니다. 이 주석이 없으면 예외로 인정하지 않습니다.
- **⚠️ 에너지 API 쿼타 및 동시성 제약:**
  - 운영계정 쿼타: `getBeElctyUsgInfo` 1,000,000회/일 (2026-06-30 업그레이드). 1회 전체 실행(36개월)으로 완료 가능.
  - **동시성 제한 필수**: `VILLAGE_SEM=1` (마을 순차), 빌딩 레벨 `Semaphore(5)`. 합산 최대 동시 호출 5개. 이를 초과 시 rate limit 429 발생.
  - 두 프로세스를 동시에 실행하면 안 됩니다 — rate limit 즉시 트리거됨.
  - **완료 판정 기준**: `_get_completed()`는 153개 레코드 + `sum(scanned_buildings_count) > 0` 조건을 함께 확인합니다. 전부 API 실패(0값)인 월은 완료로 판정하지 않아 재수집합니다.
...
**훈련 이력 파일:** `com.pulse/docs/raining-repo.md`

ML 모델 훈련, 실험, 실패, 변경이 발생한 경우 반드시 이 파일에 기록합니다. 이 파일은 훈련 이력을 누적하는 영구 로그로, 삭제하거나 초기화하지 않습니다.

### 기록 대상
- **신규 훈련**: 스크립트명, 피처 구성, 유효 마을 수, 엘보우/성능 수치, 군집 결과 요약
- **실패 시도**: 실패 원인을 데이터 문제/코드 문제/환경 문제로 구분하여 명시
- **방향 변경**: 이전 접근의 어떤 한계로 인해 변경했는지 서술
- **모델 저장 위치**: `ml_models/` 내 pkl 파일명
- **DB 반영 결과**: Alembic 마이그레이션 ID + 업데이트된 행 수

### 기록 형식
날짜별 섹션(`## YYYY-MM-DD — N차 훈련 세션`) 하위에 `[T-NN]` 번호를 붙여 각 시도를 독립 항목으로 기록합니다.

```
### [T-NN] 모델명 / 설명 — 결과 이모지 (✅ 성공 / ⚠ 주의 / ❌ 실패)
**스크립트:** scripts/train_xxx.py
**피처:** ...
**유효 마을:** N개 / 153개
**실패 원인 또는 결과 해석:** ...
```
...
### 예외
- 단순 하이퍼파라미터 조정(k값 변경 등) 재실행은 기존 항목에 추가 메모로 기록하고 새 T-NN 번호를 부여하지 않습니다.
- 파일·환경 설정만 변경한 경우(모델 재학습 없음)는 생략 가능.
